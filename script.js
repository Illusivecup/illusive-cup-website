...
class TeamsManager extends BaseManager {
    constructor(database) {
        ...
        this.demoDataAttempts = 0;
        this.maxDemoAttempts = 3;
    }
    ...
    handleTeamsUpdate(teamsData) {
        ...
        if (this.shouldCreateDemoData()) {
            this.safeCreateDemoData();
        }
        
        this.updateUI();
    }
    
    shouldCreateDemoData() {
        return Object.keys(this.teams).length === 0 && 
               !this.isCreatingDemoData && 
               this.demoDataAttempts < this.maxDemoAttempts;
    }
    ...
    async safeCreateDemoData() {
        if (this.isCreatingDemoData || this.demoDataAttempts >= this.maxDemoAttempts) {
            return;
        }
        
        this.isCreatingDemoData = true;
        this.demoDataAttempts++;
        
        try {
            console.log(`🔄 Попытка создания демо-данных (${this.demoDataAttempts}/${this.maxDemoAttempts})...`);
            await this.createDemoData();
            console.log('✅ Демо-данные успешно созданы');
        } catch (error) {
            console.error(`❌ Ошибка создания демо-данных (попытка ${this.demoDataAttempts}):`, error);
            
            if (this.demoDataAttempts >= this.maxDemoAttempts) {
                ErrorHandler.showError('Не удалось создать демо-данные после нескольких попыток');
            }
        } finally {
            this.isCreatingDemoData = false;
        }
    ...
    async createDemoData() {
        ...
        const demoTeams = {
            team1: {
                name: "Labubu Team",
                slogan: "Мы команда Labubu, мы милые такие, Но на пути к победе — мы просто стихия! Наш талисман — не просто пена для забавы, А грозный «Жумайсынба», что смоет всех, оставив лишь осколки славы!",
                players: [
        ...
        Object.keys(demoTeams).forEach(teamId => {
            ...
            team.mmr = this.calculateTeamMMR(team.players);
            console.log(`✅ ${team.name}: MMR = ${team.mmr}`);
        });

        await this.database.ref('teams').set(demoTeams);
        await this.createOtherDemoData();
    }
    
    async createOtherDemoData() {
        try {
            const demoBracket = {
                quarterfinals: [
            ...
            await this.database.ref('bracket').set(demoBracket);
            ...
            await this.database.ref('schedule').set(demoSchedule);
            ...
            await this.database.ref('tournament').set(demoTournament);
            
        } catch (error) {
            console.error('❌ Ошибка создания дополнительных демо-данных:', error);
            throw error;
        }
    }
